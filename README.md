<h1 align="center">꼬순내 신당</h1>

<p align="center">
  <img src="https://pet-fortune.com/og-image.png" alt="Pet Fortune" width="50%">
</p>

<h3 align="center">🔮 국내 최초! 반려동물을 위한 사주 기반 신점 서비스! 🔮</h3>

<p align="center">
  반려동물의 생년월일만 입력하면, 성격, 궁합, 전생까지 간단하고 귀엽게 풀이해주는<br>
  국내 유일의 LLM 기반 신점 콘텐츠!<br>
  신점 세계관 속 천년묵은 고양이 신령님이 전하는, 오직 우리 아이만을 위한 맞춤 점괘를 만나보세요 😻<br><br>
  ✨ <strong><a href="https://pet-fortune.com" target="_blank">지금 바로, 당신만의 반려동물 신점을 확인해보세요</a></strong>
</p>

<br><br><br>

## 🐾 프로젝트 소개
**꼬순내 신당**은 반려동물의 생년월일만 입력하면  
성격, 습관, 전생, 궁합, 오늘의 기분과 간식까지 귀엽고 신비롭게 알려주는 운세 서비스입니다.  

LLM 기반 자연어 생성을 통해, 보호자와 반려동물 사이의 감정 교감을 더 깊고 특별하게 만들어줍니다.  
단순한 운세 풀이를 넘어, 반려동물을 더 잘 이해하고 사랑할 수 있도록 도와주는  
**국내 최초의 반려동물 전용 신점 기반 콘텐츠 플랫폼**입니다.

- 귀엽고 감성적인 UI와 세계관  
- 현실적인 행동 분석 + 서사형 전생 스토리  
- 오늘의 간식 추천, 장난감 MBTI, 케미 해석 등  

작지만 사랑스러운 디테일로 하루를 채워드립니다.
<br><br><br>

## 📷 DEMO
| HOME | 댕냥이 신점 정보 입력 | 댕냥이 신점 결과|
|:--:|:--:|:--:|
| <img width="200" alt="image" src="https://github.com/user-attachments/assets/a7581e48-822f-4abe-85ca-1f63bfcc0a17" /> | <img width="200" alt="image" src="https://github.com/user-attachments/assets/b917762a-dd90-4710-a145-cceb6b765a13" /> | <img width="200" alt="image" src=https://github.com/user-attachments/assets/83819ed0-9041-40c7-a3f0-e66206fd3433/> |

<br><br><br>

## 📌 서비스 개요

| 항목 | 내용 |
|------|------|
| 서비스명 | **꼬순내 신당** |
| 주요 컨셉 | 반려동물의 사주를 신점 세계관으로 해석하는 국내 최초 감성 운세 서비스 |
| 콘텐츠 구성 | 사주 기반 성격 풀이, 반려동물 궁합, 반려인 궁합, 전생 이야기, 오늘의 기분 & 간식, 장난감 추천 |
| 주요 타겟 | 20~40대 반려동물 보호자 (MBTI, 궁합, 감성 콘텐츠에 관심 많은 사용자층) |
| 제공 포인트 | 생년월일만으로 반려동물의 숨겨진 매력을 간단하고 귀엽게 확인할 수 있는 직관적인 운세 콘텐츠 |
| 수익 모델 | 무료 운세 기반 → 광고 수익 + 프리미엄 리딩 유료 콘텐츠 |
| 서비스 특징 | LLM 기반 자연어 운세 생성, 마이크로서비스 아키텍처 기반 안정적 확장성, 정서적 교감을 중심으로 설계된 UI/UX |
| 마스코트 | 천년 묵은 고양이 신령 **복슬 신령님**이 반려동물의 운세를 점지해주는 귀엽고 신비로운 세계관 설정 |


<br><br><br>

## ✨ 핵심 기능
- **반려동물 사주 분석**  
  사주 기반의 해석으로 반려동물의 성격, 습관, 사랑 표현 방식,  
  사주 기반 MBTI 스타일 및 장난감 추천까지 포함된 맞춤형 운세 리포트 제공

- **반려동물 궁합 확인**  
  두 반려동물 간의 사주 궁합을 분석하여 감정 흐름, 거리감, 놀이 상호작용, 루틴 조화 등을  
  구체적이고 현실감 있게 해석하고, 케미 행동 묘사와 속마음 메시지, 공동 장난감 추천까지 포함한  
  귀엽고 생생한 궁합 콘텐츠 제공

- **반려인 궁합 분석**  
  보호자와 반려동물의 사주를 바탕으로 상호작용 스타일, 놀이 궁합, 루틴 케미를 해석하고  
  오늘의 기분 흐름과 함께 일상 속 행동 팁을 센스 있게 제안

- **반려동물 전생 탐색**  
  반려동물의 전생, 애칭, 살던 곳, 마지막 기억, 남겨진 흔적까지  
  사주 기반으로 구성된 감성적이고 신비로운 이야기 제공  
  귀엽고도 촉감 있는 표현을 통해 보호자에게 감정적 몰입을 선사

- **오늘의 기분 & 간식 추천**  
  사주의 일간 흐름을 기반으로 반려동물의 감정 상태와 어울리는 간식을 추천하여  
  하루를 더 사랑스럽게 만들어주는 콘텐츠 제공
<br><br><br>

## 🛠️ 개발 환경
### 💻 Backend

[![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.2.3-green)](https://spring.io/projects/spring-boot)
[![Java](https://img.shields.io/badge/Java-17-orange)](https://www.oracle.com/java/)
[![JPA](https://img.shields.io/badge/JPA-yellow)](https://www.oracle.com/java/technologies/persistence-jsp.html)
[![QueryDSL](https://img.shields.io/badge/QueryDSL-5.0.0-brightgreen)](http://querydsl.com/)
[![Spring Cloud](https://img.shields.io/badge/Spring%20Cloud-2023.0.0-purple)](https://spring.io/projects/spring-cloud)
[![Netflix Eureka](https://img.shields.io/badge/Netflix%20Eureka-blue)](https://github.com/Netflix/eureka)

### 🖥️ Frontend

[![Vue.js](https://img.shields.io/badge/Vue.js-3.0-green)](https://vuejs.org/)
[![Vuex](https://img.shields.io/badge/Vuex-4.0-darkgreen)](https://vuex.vuejs.org/)
[![Vue Router](https://img.shields.io/badge/Vue%20Router-4.0-yellowgreen)](https://router.vuejs.org/)
[![Axios](https://img.shields.io/badge/Axios-1.6.0-orange)](https://axios-http.com/)

### 🔧 Infrastructure

[![Docker](https://img.shields.io/badge/Docker-blue)](https://www.docker.com/)
[![Docker Compose](https://img.shields.io/badge/Docker%20Compose-lightblue)](https://docs.docker.com/compose/)
[![Jenkins](https://img.shields.io/badge/Jenkins-red)](https://www.jenkins.io/)
[![Linux](https://img.shields.io/badge/Linux-gray)](https://www.linux.org/)
<br><br><br>

## 🏗️ 아키텍처 구조
### 🧩 마이크로서비스 아키텍처  
각 기능(사주, 궁합, 전생)은 독립적인 마이크로서비스로 구성되어 있어 개별 배포와 유지보수가 유연합니다.

### ⚖️ 로드 밸런싱  
Netflix Eureka를 통한 서비스 디스커버리 및 로드 밸런싱 구현:
- 트래픽 자동 분산으로 안정적인 처리
- 인스턴스 등록/해제 자동화
- 장애 복원력 향상

### 🤖 AI 기반 운세 콘텐츠 생성  
OpenAI GPT 기반 LLM API를 활용하여 콘텐츠를 생성:
- 반려동물 특성 기반 맞춤 운세 제공
- 자연스럽고 몰입감 있는 텍스트 출력
<br><br><br>

## 📊 성능 및 안정성
### 🧪 스트레스 테스트  
- JMeter를 활용한 동시 접속 부하 테스트 수행  
- 약 1,000명 동시 사용자 처리 검증  
- 로드 밸런싱을 통한 안정적 서비스 유지


## 🚀 확장성 계획

- 트래픽 급증 대응 가능한 서버 구조 마련
- 마이크로서비스 기반으로 기능 단위 확장 용이
- 향후 글로벌 사용자 대응 고려 중


## 📈 향후 개발 로드맵
- iOS 및 Android 앱 출시
- 추가 컨텐츠 개발
<br><br><br>

## 👥 Team

| 구성원 | GitHub | 역할 |
|------|------|--------|
| 배소연 | [@thdus12](https://github.com/thdus12)| 프론트엔드, 백엔드, 디자인, 기획, 프롬프트 엔지니어 |
| 주재범 | [@jaebum7396](https://github.com/jaebum7396) | 프론트엔드, 백엔드, 인프라 |

<br><br><br>

## 📞 Contact

- 📧 Email: petfortune8996@gmail.com  
- 📸 Instagram: [@pet._.fortune](https://www.instagram.com/pet._.fortune)
<br><br><br>
